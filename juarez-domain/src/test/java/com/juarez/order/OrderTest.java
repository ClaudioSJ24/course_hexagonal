...
import com.juarez.domain.entities.order.events.*;
...
public class OrderTest {
    ...
    void shouldCreateOrderWithValidDataAndRegisterOrderCreatedEvent() {
        ...
        assertInstanceOf(OrderCreated.class, order.getDomainEvents().getFirst());
    }
    ...
    void shouldCalculateTotalAmountCorrectly() {
        ...
        Money expectedTotal = items.getFirst().getSubtotal();
    ...
    void shouldConfirmOrderAndRegisterOrderConfirmedEvent() {
        ...
        // Verify event registration
        assertEquals(1, order.getDomainEvents().size());
        assertInstanceOf(OrderConfirmed.class, order.getDomainEvents().getFirst());
    }
    ...
    void shouldShipOrderAndRegisterOrderShippedEvent() {
        ...
        // Verify event registration
        assertEquals(1, order.getDomainEvents().size());
        assertInstanceOf(OrderShipped.class, order.getDomainEvents().getFirst());
    }
    ...
    void shouldDeliverOrderAndRegisterOrderDeliveredEvent() {
        ...
        // Verify event registration
        assertEquals(1, order.getDomainEvents().size());
        assertInstanceOf(OrderDelivered.class, order.getDomainEvents().getFirst());
    }
    ...
    void shouldCancelOrderAndRegisterOrderCancelledEvent() {
        ...
        // Verify event registration
        assertEquals(1, order.getDomainEvents().size());
        OrderCancelled event = (OrderCancelled) order.getDomainEvents().getFirst();
    ...
    void shouldRemoveItemFromOrderWhenStatusIsPENDING() {
        OrderRoot order = createValidOrder();
        OrderItem itemToRemove = order.getItems().getFirst();
        int initialItemCount = order.getItems().size();
        Money initialTotal = order.getTotalAmount();
